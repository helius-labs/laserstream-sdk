...
func main() {
	log.SetFlags(0)
	log.Println("📃 Entry Subscription")
	...
	// Create subscription request for entries
	commitmentLevel := laserstream.CommitmentLevel_PROCESSED
	subscriptionRequest := &laserstream.SubscribeRequest{
		Entry: map[string]*laserstream.SubscribeRequestFilterEntry{
			"all-entries": {}, // Subscribe to all entry updates
		},
	...
	dataCallback := func(data *laserstream.SubscribeUpdate) {
		log.Printf("Entry Update: %+v", data)
	}
