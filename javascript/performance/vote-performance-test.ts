...
async function runVotePerformanceTest(testDurationSeconds: number = 60) {
  console.log('🎯 VOTE PROGRAM PERFORMANCE TEST - Laserstream vs Yellowstone (HIGHEST THROUGHPUT)');
  console.log(`Duration: ${testDurationSeconds}s | Focus: Vote transactions timing comparison (500+ msgs/sec)\n`);

  // Vote Program - highest transaction volume on Solana
  ...
  const commonSubscriptionConfig = {
    accountInclude: [VOTE_PROGRAM],
    accountExclude: [],
    accountRequired: [],
    vote: true, // Include vote transactions explicitly
    failed: false
  };
  ...
  const subscriptionRequest = {
    transactions: {
      'vote-transactions': commonSubscriptionConfig
    },
  ...
  console.log(`📊 Expected throughput: 500+ vote transactions per second`);
  console.log(`🎯 Target: ${VOTE_PROGRAM} (Vote Program)`);
  console.log('⚠️  WARNING: Very high throughput - may overwhelm console output');
  ...
  const laserstreamStream = await laserstreamClient.subscribe(
    ...
    (error: Error | null, update: any) => {
      ...
      if (message.yellowstoneTime) {
        ...
        // Only log VERY significant differences to avoid console spam
        if (Math.abs(message.timeDifference) > 50) {
  ...
  // Progress updates with enhanced diagnostics (less frequent due to high throughput)
  const progressInterval = setInterval(() => {
    ...
    console.log(`📊 Vote Transactions: LS ${metrics.laserstreamMessages.toLocaleString()} | YS ${metrics.yellowstoneMessages.toLocaleString()} | Shared ${metrics.sharedMessages.toLocaleString()}`);
    ...
    console.log(`🔍 LS-only: ${lsOnlyMessages.toLocaleString()} | YS-only: ${ysOnlyMessages.toLocaleString()} | Share rate: ${shareRate.toFixed(1)}%`);
    console.log(`🚀 Throughput: LS ${metrics.laserstreamThroughput.toFixed(0)}/sec | YS ${metrics.yellowstoneThroughput.toFixed(0)}/sec`);
  ...
  }, 10000); // Less frequent updates due to very high throughput
  ...
  function generateFinalReport() {
    ...
    console.log('\n\n🏁 VOTE PROGRAM PERFORMANCE REPORT');
    ...
    console.log(`📊 Data Type: Vote transactions (HIGHEST throughput)`);
    console.log(`🎯 Target Program: ${VOTE_PROGRAM}`);
    console.log();

    // Message Counts with Enhanced Analysis
    console.log('📈 VOTE TRANSACTION COUNTS:');
    ...
    if (shareRate < 50) {
      ...
      console.log('💡 High throughput may cause network congestion differences');
    }
    ...
    // Throughput (very important for vote transactions)
    console.log('🚀 THROUGHPUT (CRITICAL METRIC):');
    console.log(`🟦 Laserstream: ${metrics.laserstreamThroughput.toFixed(0)} vote transactions/second`);
    console.log(`🟨 Yellowstone: ${metrics.yellowstoneThroughput.toFixed(0)} vote transactions/second`);
    
    if (metrics.laserstreamThroughput > 0 && metrics.yellowstoneThroughput > 0) {
      const throughputAdvantage = ((metrics.laserstreamThroughput / metrics.yellowstoneThroughput - 1) * 100);
      if (Math.abs(throughputAdvantage) > 5) {
        const winner = throughputAdvantage > 0 ? 'Laserstream' : 'Yellowstone';
        console.log(`⚡ ${winner} has ${Math.abs(throughputAdvantage).toFixed(1)}% throughput advantage`);
      }
    }
    console.log();

    // Enhanced Timing Analysis (Core Metric)
    if (metrics.sharedMessages > 50) { // Higher threshold for vote transactions due to high volume
      ...
      console.log(`📊 Sample size adequacy: ${metrics.sharedMessages >= 1000 ? '✅ Excellent' : metrics.sharedMessages >= 100 ? '✅ Good' : '⚠️ Small'} (${metrics.sharedMessages} samples)`);
      
      if (Math.abs(metrics.averageTimeDiff) > marginOfError && metrics.sharedMessages >= 100) {
        ...
        if (metrics.sharedMessages < 100) {
          console.log(`   (Sample size too small: ${metrics.sharedMessages} < 100)`);
        }
      }
      console.log();
      ...
      if (shareRate < 30) {
        ...
        console.log('💡 High throughput may cause network congestion - consider lower volume tests');
        ...
        console.log('💡 High throughput test - results may vary due to network congestion');
      }
    } else {
      console.log('❌ INSUFFICIENT SHARED TRANSACTIONS - Cannot perform timing analysis');
      console.log(`   Only ${metrics.sharedMessages} shared transactions (minimum: 50 for high-throughput test)`);
      console.log('💡 Very high throughput may cause clients to see different transaction sets');
    }

    // High-throughput specific warnings
    console.log('\n⚠️  HIGH THROUGHPUT TEST NOTES:');
    console.log('📊 Vote transactions are the highest volume on Solana');
    console.log('🌐 Network congestion may cause message delivery variations');
    console.log('🔄 Consider multiple test runs for statistical reliability');
    console.log('💡 Lower throughput tests may provide more stable timing measurements');
  ...
  console.log('✅ Both clients connected! Starting VOTE PROGRAM performance measurement...');
  console.log(`⏱️  Test will run for ${testDurationSeconds} seconds`);
  console.log('🎯 Focus: Vote transaction timing comparison (highest throughput)');
  console.log('🔧 Same endpoints, same commitment, same timing baseline');
  console.log('⚠️  Console output will be minimal due to very high message volume');
...
runVotePerformanceTest(testDuration).catch((error) => {
  console.error('💥 Vote program performance test failed:', error);
  process.exit(1);
}); 